"""
Load a per-user show file server path for the current Unreal project.
...
WBP_00_InitializeProject to populate the saved show file server path.

Blueprint usage:
    import get_file_server_path
    import importlib
    importlib.reload(get_file_server_path)
    show_file_server_path = get_file_server_path.run()

Returns:
    The saved show file server path as a string, or "" if not found.
"""
...
def run():
    try:
        ...
        if value:
            ...
            _log(f"Loaded ShowFileServerPath='{value}'")
            return value
