...
def _derive_render_name_from_graph_name(value):
    graph_name = _sanitize_graph_name(value)
    if not graph_name:
        return ""
...
def _build_render_output_data(output_root, shot_name, movie_render_graph_name):
    ...
    render_name = _derive_render_name_from_graph_name(movie_render_graph_name)
    if not render_name:
        raise ValueError("Could not derive render name from Movie Render Graph name.")

    stem_prefix = f"{shot_name}_{render_name}"
