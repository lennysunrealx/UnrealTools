"""
Persist a per-user render output folder for the current Unreal project.
...
Blueprint usage:
    ...
    import importlib
    importlib.reload(set_outputFolder)
    result = set_outputFolder.run(output_path)
...
def _normalize_path(output_path):
...
def run(output_path):
    try:
        _log(f"Raw output_path input: {output_path}")

        normalized_path = _normalize_path(output_path)
        _log(f"Normalized output_path: {normalized_path}")

        if not normalized_path:
            _log_error("output_path was empty.")
            return ""
