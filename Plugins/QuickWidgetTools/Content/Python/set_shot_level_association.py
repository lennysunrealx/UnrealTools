...
def _to_package_path(level_path):
    value = _sanitize_level_path(level_path)
    if not value:
        return ""
...
def run(show_name, sequence_name, shot_name, level_path):
    ...
    try:
        ...
        clean_level_package_path = _to_package_path(clean_level_path)
        ...
        current_path_string_package = _to_package_path(current_path_string)
        incoming_path_string_package = clean_level_package_path
        ...
        _log(f"Incoming {_PATH_STRING_PROPERTY_NAME}: {incoming_path_string_package!r}")

        level_matches = (current_normalized == incoming_normalized)
        string_matches = (current_path_string_package == incoming_path_string_package)

        if level_matches and string_matches:
            _log("Path compare result: matched")
            _log("Update skipped: existing level association and path string already match.")
            _log("Final return value: True")
            return True

        if not level_matches:
            _log("Path compare result: level association differed")
            ...
            _log("Level association already matched. Skipping level property update.")

        if not string_matches:
            if not _set_path_string_value(shot_data_asset, incoming_path_string_package):
                _log("Final return value: False")
                return False
            _log(f"Update applied: {_PATH_STRING_PROPERTY_NAME} updated.")
        else:
            _log(f"{_PATH_STRING_PROPERTY_NAME} already matched. Skipping string update.")
