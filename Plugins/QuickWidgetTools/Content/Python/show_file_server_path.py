"""
Persist a per-user show file server path for the current Unreal project.

Writes to:
    <Project>/Saved/Config/WindowsEditor/QuickWidgetToolsSettings.ini

Intended for use from an Execute Python Script node, usually from
WBP_00_InitializeProject after choosing the top-level show folder.

Blueprint usage:
    import show_file_server_path
    import importlib
    importlib.reload(show_file_server_path)
    result = show_file_server_path.run(show_file_server_path)
...
def _log_manifest_status(normalized_show_path):
    manifest_path = os.path.join(normalized_show_path, _SHOW_MANIFEST_FILE_NAME)
    ...
    if os.path.exists(manifest_path):
        ...
        _log_warning(
            f"Show manifest was not found at '{manifest_path}'. "
            "Path will still be saved."
        )


def run(show_path):
    try:
        _log(f"Raw show_path input: {show_path}")

        normalized_path = _normalize_path(show_path)
        _log(f"Normalized show_path: {normalized_path}")

        if not normalized_path:
            _log_error("show_path was empty.")
            return ""

        _log_manifest_status(normalized_path)
        ...
        updated_text = _upsert_section_key(before_text, _SECTION, _KEY, normalized_path)
        _write_text_file(settings_path, updated_text)

        after_text = _read_text_file(settings_path)
        _log(f"Read {len(after_text)} characters from settings file after write.")

        verified_value = _get_section_value(after_text, _SECTION, _KEY)
        if verified_value != normalized_path:
            _log_error(
                f"Post-write verification failed. Expected '{normalized_path}' but found '{verified_value}'"
            )
            return ""
